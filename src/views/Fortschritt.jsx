...
export default function Fortschritt({ erledigt = {}, lernschritte = {}, aufgaben = [] }) {
  ...
  const aktiveLernwege = faecher
    .map((f) => {
      ...
      if (!akt) return null;
      const stand = lernwegStand(akt.schritte);
      const naechster = akt.schritte.find((s) => !s.fertig);
      return {
        fach: f.fach,
        thema: akt.t.label,
        fertig: stand.fertig,
  ...
  const mitBonus = faecherKompetenz.map((f) => {
  ...
  return (
    <div className="view">
      ...
      {/* Lernwege (aktiver Lernweg je Fach, direkt aus dem Wissen-Netz) */}
      <section className="block">
        <Label>Deine aktiven Lernwege</Label>
